...
| 功能 | 说明 |
|------|------|
| 自动记账（私聊/群聊/转发） | 私聊或群聊发送金额、或转发消息 → 自动提取金额 → 自动入账 |
...
| 推送时间设置 | 管理员与普通用户都可在私聊通过内联按钮设置自己的每日/月度推送时间 |
| 关键词别名 | 可绑定并在关键词列表中通过内联按钮删除用户/项目关键词（非管理员仅显示自己的配置） |
| 权限控制 | 管理员白名单；可选用户/群白名单；有权限的管理员/普通用户可将机器人拉群并自动加入群白名单，退群自动移除 |
...
> 管理员在私聊发送 `/start` 后，可通过内联按钮完成：用户关键词绑定/删除、项目关键词绑定/删除、统计查看、可用用户权限增删查、清理用户记账、推送时间设置等全部流程。  
> 非管理员（被添加用户）在私聊可绑定/查看自己的项目关键词、清空自己的记账、设置自己的推送时间，并且统计仅展示自己的记账数据。
...
支持识别正负金额：如 `100-20`、`减20`，会按加减结果入账。

### 来源识别优先级

1. 非转发消息：直接使用当前发消息用户作为记账来源
2. 转发来源用户的 **Telegram 用户 ID**（最可靠）
3. 转发来源的 **显示名称**
4. 通过 `/bindid` 绑定的 **关键词别名**
